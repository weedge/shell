...
seq_num=`seq 0 14`
for i in $seq_num; do
	mysqldump -h***** -utuanuser -ptuandev123 tuan --table online_data > create_online_data_table.sql
	...
	#echo $sql
	mysql -h****** -utuanuser -ptuandev123 tuan -e "${sql}"
	del_sql='DROP TABLE IF EXISTS online_td_'${i}';'
	mysql -h****** -utuanuser -ptuandev123 tuan -e "${del_sql}"
done
